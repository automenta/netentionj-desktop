...
public class ServiceTest {
    ...
    public void testService() throws SocketException, UnknownHostException, InterruptedException {
        ...
        final Network svc1 = new Network(addr1, new InetSocketAddress[] { addr2 });
        final Network svc2 = new Network(addr2, new InetSocketAddress[] { addr1, addr2 });
        final CountDownLatch latch = new CountDownLatch(2);

        svc1.listen("topic.", new Listener() {
            @Override
        ...
        svc2.listen("topic.", new Listener() {
            @Override
            public void handleMessage(final String topic, final String message) {
    ...
    public void testServiceMore() throws SocketException, UnknownHostException, InterruptedException {
        final InetAddress localhost = InetAddress.getByName("localhost");
        final Network[] services = new Network[TREAD_COUNT];
        ...
        for (int idx = 0; idx < TREAD_COUNT; idx++) {
            ...
            final Network svc = new Network(addr, new InetSocketAddress[] { seed });

            services[idx] = svc;

            svc.listen("topic.", new Listener() {
                @Override
                public void handleMessage(final String topic, final String message) {
                    endLatch.countDown();
